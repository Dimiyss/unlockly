...
public class WalletManager: ObservableObject {
    ...
    public init(userDefaults: UserDefaults = UserDefaults(suiteName: "group.com.arhiplabs.unstuckly") ?? .standard) {
        self.userDefaults = userDefaults
        if let data = userDefaults.data(forKey: "unstuckly_wallet"),
        ...
        if let ruleData = userDefaults.data(forKey: "unstuckly_active_rule"),
    ...
    public func saveActiveRule(_ rule: Rule) {
        ...
        if let encoded = try? JSONEncoder().encode(rule) {
            userDefaults.set(encoded, forKey: "unstuckly_active_rule")
        }
    }
    ...
    private func saveWallet(_ wallet: Wallet, to defaults: UserDefaults) {
        if let encoded = try? JSONEncoder().encode(wallet) {
            defaults.set(encoded, forKey: "unstuckly_wallet")
        }
    }
}
